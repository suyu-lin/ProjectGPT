...
function App() {
  ...
  useEffect(() => {
    const setup = async () => {
      try {
        ...
        // Model ko memory mein load karein [cite: 76, 146]
  ...
  const handleAction = useCallback(async (actionType: 'debug' | 'explain' | 'optimize') => {
    if (!code.trim()) return;
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto', fontFamily: 'sans-serif' }}>
      <header style={{ marginBottom: '20px', borderBottom: '1px solid #eee', paddingBottom: '10px' }}>
        ...
        {progress > 0 && progress < 100 && (
          <div style={{ width: '100%', backgroundColor: '#eee', height: '10px', borderRadius: '5px' }}>
            <div style={{ width: `${progress}%`, backgroundColor: '#007bff', height: '10px', borderRadius: '5px' }}></div>
          </div>
        )}
      </header>
      ...
      <textarea
        ...
        style={{ width: '100%', height: '200px', marginBottom: '10px', fontFamily: 'monospace', padding: '10px' }}
      />

      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
        <button onClick={() => handleAction('debug')} disabled={!ready || isProcessing}>
          {isProcessing ? 'Thinking...' : 'Debug'}
        </button>
        <button onClick={() => handleAction('explain')} disabled={!ready || isProcessing}>
          {isProcessing ? 'Thinking...' : 'Explain'}
        </button>
        <button onClick={() => handleAction('optimize')} disabled={!ready || isProcessing}>
          {isProcessing ? 'Thinking...' : 'Optimize'}
        </button>
      </div>

      <div style={{ background: '#2d3436', color: 'white', padding: '15px', borderRadius: '5px', minHeight: '100px', whiteSpace: 'pre-wrap' }}>
